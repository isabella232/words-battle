...
//				this.connection.println("Received your message");
	...
	private void HandleMessage(ClientMessage msg) {
		if (!this.userIsRegistered && msg.getType()!= ClientMessageType.REGISTER_PLAYER_NAME) {
			LOGGER.warn("User tries to do something without registration!");
			return;
		}
		switch (msg.getType()) {
		case REGISTER_PLAYER_NAME: {
			String PlayerName = msg.getPlayerName();
			LOGGER.info("User tries to register name: " + PlayerName);
		...
		case REQUEST_GAME: {
			...
			LOGGER.info("User requests game with opponent: " + opponentName);
		...
		case DENY_GAME_REQUEST: {
			...
			LOGGER.info("Player " + playerName + "denied game request from opp " + opponentName);
		...
		case ACCEPT_GAME_REQUEST: {
			String playerName = msg.getPlayerName();
			String opponentName = this.player.getName();
			LOGGER.info("Player " + playerName + "accepted game request from opp " + opponentName);
